...
export function renderArticleCard(article, site) {
  const { slug, title, date, description, tags } = article;
  const dateEl = date
    ? `<time class="card-date" datetime="${escape(date)}">${escape(formatDateShort(date, site.localeCode))}</time>`
    : "";

  const tagAttr = (Array.isArray(tags) ? tags : []).join("|");

  return `<a class="card" href="${escape(url(site, `/articles/${slug}/`))}" data-tags="${escape(tagAttr)}">
...
export function renderProjectCard(project, site) {
  const { name, url: href, tags, status } = project;
  const description = pick(project.description, site.localeCode);

  // The class keeps the raw status so the styling stays locale-independent;
  // only the visible label is translated.
  const statusEl = status
    ? `<span class="status status--${escape(String(status).toLowerCase().replace(/\s+/g, "-"))}">${escape(site.status?.[status] ?? status)}</span>`
    : "";
  ...
  return `<${tag} class="card${href ? "" : " card--static"}"${linkAttrs}>
...
const FLAGS = {
  pt: {
    label: { en: "In Portuguese", pt: "Em português" },
  ...
  en: {
    label: { en: "In English", pt: "Em inglês" },
...
function renderFlag(lang, site) {
  const flag = FLAGS[lang];
  if (!flag) return "";
  return `<span class="flag" role="img" aria-label="${escape(pick(flag.label, site.localeCode))}">${flag.svg}</span>`;
}

export function renderPodcastCard(episode, site) {
  const { title, date, duration, url: href, show, platform, lang } = episode;
  const description = pick(episode.description, site.localeCode);
  const dateEl = date
    ? `<time class="card-date" datetime="${escape(date)}">${escape(formatDateShort(date, site.localeCode))}</time>`
    : "";
  ...
  <h3 class="card-title">${escape(title)}</h3>
  ${description ? `<p class="card-description">${escape(description)}</p>` : ""}
  ${cardFooter(dateEl + renderFlag(lang, site) + showEl + durationEl + platformEl)}
</a>`;
}

export function renderGearCard(item, site) {
  const { name, url: href, image, affiliate, links = [] } = item;
  const description = pick(item.description, site.localeCode);
  ...
  const title = href
    ? `<a class="gear-link" href="${escape(href)}" target="_blank" rel="${rel}">${escape(name)}</a>`
    : escape(name);
  ...
  const extras = links.length
    ? `<div class="gear-links">${links
        .map(
          (link) =>
            `<a class="gear-chip" href="${escape(link.href)}" target="_blank" rel="noopener noreferrer">${escape(pick(link.label, site.localeCode))}</a>`
        )
        .join("")}</div>`
    : "";
